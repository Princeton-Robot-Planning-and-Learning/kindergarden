...
def test_config_rejects_anatomically_impossible_configurations(field):
    """A hyperextended elbow is caught at construction, goal included.

    Nothing downstream would catch the goal: the limb is only ever built at its initial
    configuration, so an impossible goal would just make the task unsolvable.
    """
...
def test_human_scene_torso_stays_where_it_is_put(physics_client_id):
    """The torso holds its pose under gravity, so the limbs stay on a whole person.

    It is loaded with a fixed base, which PyBullet implements as a zero-mass base link.
    Giving that link the person's torso mass would make the body dynamic and it would
    fall away from the scene, taking the limbs' geometry out of correspondence with it.
    """
    config = _make_config(scene_type="human", limb_name="human-left-arm")
    scene = create_scene(config, physics_client_id)
