...
def test_tidybot3d_observation_space():
    ...
    observation space."""
    env = ObjectCentricTidyBot3DEnv(num_objects=3, task_config_path=str(_TEST_TASKS / "tidybot-ground-o3.json"))
    obs = env.reset()[0]
...
def test_tidybot3d_env_object_centric_state():
    ...
    num_objects = 3
    env = ObjectCentricTidyBot3DEnv(num_objects=num_objects, task_config_path=str(_TEST_TASKS / "tidybot-ground-o3.json"))
    obs, _ = env.reset()
...
def test_tidybot3d_env_set_state():
    ...
    states = []
    actions = []
    env = ObjectCentricTidyBot3DEnv(num_objects=3, allow_state_access=True, task_config_path=str(_TEST_TASKS / "tidybot-ground-o3.json"))
