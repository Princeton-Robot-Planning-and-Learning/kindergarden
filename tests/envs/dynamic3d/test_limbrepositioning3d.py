...
def test_initial_state_within_joint_ranges(variant):
    ...
    try:
        ...
        for seed in range(5):
            ...
            assert environment.limb_joint_limit_violation() == 0.0
    finally:
        environment.close()
...
def test_variant_tasks_are_anatomically_possible(variant):
    ...
    A goal outside it would be unreachable without bending the joint the wrong way, and
    an experiment over several people needs the shipped tasks to fit each of them.
    """
...
def test_a_variant_can_be_given_an_asymmetric_person():
    """The limb being repositioned takes its own range of motion, not the person's."""
...
def test_joint_limit_violations_are_reported():
    """The limb URDFs use continuous joints, so only this reports a bent-back joint."""
    environment = ObjectCentricLimbRepositioning3DEnv(variant="wheelchair-left-arm")
    try:
        environment.reset(seed=0)
        assert environment.limb_joint_limit_violation() == 0.0
        hyperextended = list(environment.limb.get_joint_positions())
        hyperextended[3] = -0.5
        environment.limb.set_joints(hyperextended)
        assert np.isclose(environment.limb_joint_limit_violation(), 0.5)
    finally:
        environment.close()
